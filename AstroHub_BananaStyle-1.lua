...
BringEnemy = function(Mon)
    ...
    local maxBring = tonumber(_G.BringCount) or 6
    if maxBring < 1 then maxBring = 1 end
    if maxBring > 6 then maxBring = 6 end
    ...
    local function aliveRoot(enemy)
        local hum = enemy:FindFirstChildOfClass("Humanoid")
        local root = enemy:FindFirstChild("HumanoidRootPart")
        if hum and root and hum.Health > 0 then
            return root, hum
    ...
    pcall(function()
        ...
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
            if v.Name == monName then
                local root, hum = aliveRoot(v)
                if root then
                    local d = (root.Position - targetPos).Magnitude
                    if d <= 3500 then
                        table.insert(candidates, {model = v, root = root, hum = hum, dist = d})
                    end
                end
            end
        end
        ...
        local brought = 0
        for i = 1, math.min(maxBring, #candidates) do
            ...
            if hasNet(root) or e.dist > 12 then
                root.CFrame = CFrame.new(targetPos + Vector3.new((i % 3) * 1.2, 0, math.floor((i - 1) / 3) * 1.2))
            end
...
local C = {
    ...
    select = Color3.fromRGB(255, 170, 45),
...
function AstroUI:AddTab(opts)
    ...
    function tabProxy:AddButton(opts)
        ...
        b.Parent = r
        corner(b, 8)
        -- VERY heavy black glow layers
        for i, tr in ipairs({0.02, 0.08, 0.15, 0.25, 0.4, 0.55}) do
            local g = Instance.new("ImageLabel")
            g.BackgroundTransparency = 1
            g.Image = "rbxassetid://5028857084"
            g.ImageColor3 = Color3.fromRGB(0, 0, 0)
        ...
        tStroke.Color = Color3.fromRGB(240, 245, 255)
        tStroke.Thickness = 1.1
    ...
    if BananaLib and BananaLib.CreateMain then
        local ok, main = pcall(function()
            return BananaLib.CreateMain({
                Title = opts.Title or "Astro Hub",
                Desc = opts.SubTitle or " - Blox Fruit",
            })
        end)
...
Tabs.HopBoss:AddButton({
    ...
    Callback = function()
        _G.HopBossActive = false
        pcall(function()
            Fluent:Notify({Title = "Hop Boss", Content = "Stopped.", Duration = 2})
        end)
    end
})
