--[[
Astro Hub - Blox Fruits
Made by: boesesmonster (thank azeox main functions)
Hybrid: Original W-Azeox logic + Luna UI
]]
...
BringEnemy = function(Mon)
    ...
    local maxBring = math.clamp(tonumber(_G.BringCount) or 6, 1, 6)
    ...
    pcall(function()
        ...
        local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
        local targetPos = (hrp and hrp.Position) or Mon.HumanoidRootPart.Position
        local brought = 0

        -- include main mon first
        local list = {}
        ...
        for _, v in ipairs(list) do
            ...
            if alive then
                ...
                if distance <= 3000 then
                    ...
                    -- stack under player so all stay in hitbox
                    if Network(root) or distance > 8 then
...
-- Astro config save/load
local HttpService = game:GetService("HttpService")
local ASTRO_FOLDER = "AstroHub"
local ASTRO_FILE = (game.Players.LocalPlayer and game.Players.LocalPlayer.Name or "Player") .. "-AstroHub.json"
_G.AstroConfig = _G.AstroConfig or {}
function AstroSave(key, value)
    _G.AstroConfig[key] = value
    pcall(function()
        if isfolder and not isfolder(ASTRO_FOLDER) then makefolder(ASTRO_FOLDER) end
        if writefile then
            writefile(ASTRO_FOLDER .. "/" .. ASTRO_FILE, HttpService:JSONEncode(_G.AstroConfig))
        end
    end)
end
function AstroLoad()
    local ok, data = pcall(function()
        if isfolder and not isfolder(ASTRO_FOLDER) then makefolder(ASTRO_FOLDER) end
        if readfile and isfile and isfile(ASTRO_FOLDER .. "/" .. ASTRO_FILE) then
            return HttpService:JSONDecode(readfile(ASTRO_FOLDER .. "/" .. ASTRO_FILE))
        end
    end)
    if ok and type(data) == "table" then
        _G.AstroConfig = data
        if data.Toggle_Bring_Mobs ~= nil then _B = data.Toggle_Bring_Mobs end
        if data.Slider_Bring_Count ~= nil then _G.BringCount = data.Slider_Bring_Count end
        if data.Toggle_Reset_Teleport ~= nil then _G.ResetTeleport = data.Toggle_Reset_Teleport end
        if data.Dropdown_Quest_Mode ~= nil then _G.QuestMode = data.Dropdown_Quest_Mode end
        if data.Toggle_Fast_Attack ~= nil then _G.Seriality = data.Toggle_Fast_Attack end
    end
end
pcall(AstroLoad)

-- Use custom Astro UI (marine theme). Banana lib kept optional off — yellow theme was overriding.
local BananaLib = nil
--[[
do
    local ok, res = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/obiiyeuem/vthangsitink/refs/heads/main/zzzz.lua"))()
    end)
    if ok and type(res) == "table" and res.CreateMain then
        BananaLib = res
    end
end
]]
...
local C = {
    bg = Color3.fromRGB(6, 7, 9),
    panel = Color3.fromRGB(10, 11, 13),
    side = Color3.fromRGB(8, 9, 11),
    row = Color3.fromRGB(16, 17, 20),
    row2 = Color3.fromRGB(20, 21, 25),
    text = Color3.fromRGB(245, 248, 255),
    muted = Color3.fromRGB(120, 128, 140),
    marine = Color3.fromRGB(0, 70, 160),
    marineHi = Color3.fromRGB(20, 110, 210),
    select = Color3.fromRGB(20, 110, 210),
...
local function heavyBlackGlow(parent)
    for i, t in ipairs({0.05, 0.18, 0.32, 0.48}) do
        local g = Instance.new("ImageLabel")
        g.Name = "HeavyBlackGlow" .. i
        ...
        g.Position = UDim2.fromOffset(-pad, -pad)
        g.ZIndex = math.max(1, (parent.ZIndex or 5) - 1)
        g.Parent = parent
    end
end

-- ========== Fluent-compatible UI (Banana lib bridge OR fallback) ==========
local Fluent = {}
Fluent.Options = {}
local _UI = nil
local _BananaMain = nil
local _BananaPages = {}

local function makeTabProxyFromBanana(page)
    local currentSection = page.CreateSection("Main")
    local tabProxy = {}
    function tabProxy:AddSection(name)
        currentSection = page.CreateSection(tostring(name or "Section"))
        return tabProxy
    end
    function tabProxy:AddToggle(flag, opts)
        opts = type(flag) == "table" and flag or (opts or {})
        if type(flag) == "table" then opts = flag end
        local title = opts.Title or opts.Name or tostring(flag)
        local def = opts.Default == true or opts.CurrentValue == true
        if _G.AstroConfig and type(flag) == "string" and _G.AstroConfig[flag] ~= nil then
            def = _G.AstroConfig[flag] == true
        end
        local cb = opts.Callback or function() end
        currentSection.CreateToggle({Title = title, Desc = opts.Description, Default = def}, function(v)
            if type(flag) == "string" then AstroSave(flag, v) end
            task.spawn(function() pcall(cb, v) end)
        end)
        return { SetValue = function(_, v) pcall(cb, v) end }
    end
    function tabProxy:AddButton(opts)
        opts = opts or {}
        currentSection.CreateButton({Title = opts.Title or opts.Name or "Click"}, function()
            task.spawn(function() pcall(opts.Callback or function() end) end)
        end)
        return {}
    end
    function tabProxy:AddDropdown(flag, opts)
        opts = type(flag) == "table" and flag or (opts or {})
        if type(flag) == "table" then opts = flag end
        local title = opts.Title or tostring(flag)
        local values = opts.Values or opts.Options or {}
        local def = opts.Default
        if type(def) == "table" then def = def[1] end
        local cb = opts.Callback or function() end
        if currentSection.CreateDropdown then
            pcall(function()
                currentSection.CreateDropdown({Title = title, List = values, Default = def}, function(v)
                    if type(flag) == "string" then AstroSave(flag, v) end
                    task.spawn(function() pcall(cb, v) end)
                end)
            end)
        else
            currentSection.CreateButton({Title = title .. " (set in config)"}, function() end)
        end
        return {}
    end
    function tabProxy:AddSlider(flag, opts)
        opts = type(flag) == "table" and flag or (opts or {})
        if type(flag) == "table" then opts = flag end
        local title = opts.Title or tostring(flag)
        ...
        local cb = opts.Callback or function() end
        if currentSection.CreateSlider then
            pcall(function()
                currentSection.CreateSlider({Title = title, Min = minv, Max = maxv, Default = def}, function(v)
                    if type(flag) == "string" then AstroSave(flag, v) end
                    task.spawn(function() pcall(cb, v) end)
                end)
            end)
        else
            currentSection.CreateToggle({Title = title .. " = " .. tostring(def), Default = false}, function() end)
        end
        return {}
    end
    function tabProxy:AddParagraph(opts)
        opts = opts or {}
        local label
        if currentSection.CreateLabel then
            label = currentSection.CreateLabel({Title = (opts.Title or "") .. " | " .. (opts.Content or opts.Text or "")})
        end
        local proxy = {}
        function proxy:Set(v)
            local content = ""
            if type(v) == "table" then content = tostring(v.Content or v.Text or "")
            else content = tostring(v or "") end
            if label and label.Set then pcall(function() label:Set(content) end) end
        end
        function proxy:SetDesc(t) proxy:Set(t) end
        return proxy
    end
    function tabProxy:AddInput(flag, opts)
        opts = type(flag) == "table" and flag or (opts or {})
        if type(flag) == "table" then opts = flag end
        local cb = opts.Callback or function() end
        if currentSection.CreateTextBox or currentSection.CreateInput then
            local fn = currentSection.CreateTextBox or currentSection.CreateInput
            pcall(function()
                fn({Title = opts.Title or tostring(flag), Default = opts.Default or ""}, function(t)
                    task.spawn(function() pcall(cb, t) end)
                end)
            end)
        end
        return {}
    end
    function tabProxy:AddKeybind() return {} end
    return tabProxy
end

-- Fallback custom UI (if Banana fails to load)
local AstroUI = {}
AstroUI.__index = AstroUI

function AstroUI.new(opts)
    opts = opts or {}
    local self = setmetatable({}, AstroUI)
    self.tabs = {}
    for _, g in pairs(guiParent:GetChildren()) do
        if g.Name == "AstroHub_BananaUI" then g:Destroy() end
    end
    local sg = Instance.new("ScreenGui")
    sg.Name = "AstroHub_BananaUI"
    ...
    title.TextSize = 14
    title.TextColor3 = C.marineHi
    title.Text = opts.Title or "Astro Hub - Blox Fruit"
    title.Parent = top
    ...
    side.Size = UDim2.new(0, sideW, 1, -40)
    side.Position = UDim2.fromOffset(8, 36)
    ...
    sideScroll.Size = UDim2.new(1, -6, 1, -8)
    sideScroll.Position = UDim2.fromOffset(3, 4)
    ...
    content.Size = UDim2.new(1, -(sideW + 20), 1, -40)
    content.Position = UDim2.fromOffset(sideW + 12, 36)
    ...
    tbtn.Size = UDim2.fromOffset(36, 36)
    tbtn.Position = UDim2.new(0, 10, 0.5, -18)
    tbtn.BackgroundColor3 = C.black
    tbtn.Text = ""
    ...
    dot.BackgroundColor3 = C.marineHi
    ...
    tbtn.MouseButton1Click:Connect(function()
        isOpen = not isOpen
        main.Visible = isOpen
        dot.BackgroundColor3 = isOpen and C.marineHi or C.muted
    end)
    return self
end

function AstroUI:AddTab(opts)
    opts = opts or {}
    local name = tostring(opts.Title or opts.Name or "Tab"):gsub("^Tab ", "")
    ...
    indicator.BackgroundColor3 = C.select
    ...
    page.Size = UDim2.new(1, -12, 1, -32)
    page.Position = UDim2.fromOffset(6, 30)
    ...
    local tab = { btn = btn, page = page, name = name, indicator = indicator, lbl = lbl }
    ...
    local function makeLabel(parent, text, rightSpace)
        local l = Instance.new("TextLabel")
        l.BackgroundTransparency = 1
        l.Size = UDim2.new(1, -(rightSpace or 80), 1, 0)
    ...
    function tabProxy:AddSection(secName)
    ...
    function tabProxy:AddToggle(flag, opts)
        ...
        local title = opts.Title or opts.Name or tostring(flag)
        local cb = opts.Callback or function() end
        local on = opts.Default == true or opts.CurrentValue == true
        if type(flag) == "string" and _G.AstroConfig and _G.AstroConfig[flag] ~= nil then
            on = _G.AstroConfig[flag] == true
        end
        local r = makeRow(34)
        makeLabel(r, title, 48)
        local box = Instance.new("TextButton")
        box.Size = UDim2.fromOffset(20, 20)
        box.Position = UDim2.new(1, -32, 0.5, -10)
        box.BackgroundColor3 = on and C.marineHi or Color3.fromRGB(10, 12, 16)
        box.Text = ""
        ...
        st.Color = on and C.marineHi or C.marine
        st.Thickness = 1.8
        st.Parent = box
        local tg = Instance.new("ImageLabel")
        tg.BackgroundTransparency = 1
        tg.Image = "rbxassetid://5028857084"
        tg.ImageColor3 = C.marineHi
        tg.ImageTransparency = on and 0.3 or 0.85
        tg.ScaleType = Enum.ScaleType.Slice
        tg.SliceCenter = Rect.new(24, 24, 276, 276)
        tg.Size = UDim2.new(1, 18, 1, 18)
        tg.Position = UDim2.fromOffset(-9, -9)
        tg.ZIndex = 4
        tg.Parent = box
        local function setVisual(state)
            box.BackgroundColor3 = state and C.marineHi or Color3.fromRGB(10, 12, 16)
            st.Color = state and C.marineHi or C.marine
            tg.ImageTransparency = state and 0.3 or 0.85
        end
        box.MouseButton1Click:Connect(function()
            on = not on
            setVisual(on)
            if type(flag) == "string" then AstroSave(flag, on) end
            task.spawn(function() pcall(cb, on) end)
        end)
        return { SetValue = function(_, v) on = v and true or false setVisual(on) task.spawn(function() pcall(cb, on) end) end }
    end

    function tabProxy:AddButton(opts)
        opts = opts or {}
        local r = makeRow(36)
        makeLabel(r, opts.Title or opts.Name or "Button", 86)
        local b = Instance.new("TextButton")
        b.Size = UDim2.fromOffset(70, 28)
        b.Position = UDim2.new(1, -82, 0.5, -14)
        b.BackgroundColor3 = C.marine
        b.Text = "Click"
        ...
        corner(b, 8)
        -- marine glow layers
        for i, tr in ipairs({0.25, 0.4, 0.55, 0.7}) do
            local g = Instance.new("ImageLabel")
            g.BackgroundTransparency = 1
            g.Image = "rbxassetid://5028857084"
            g.ImageColor3 = C.marineHi
    ...
    function tabProxy:AddDropdown(flag, opts)
        ...
        local selected
        if type(opts.Default) == "table" then selected = tostring(opts.Default[1] or "")
        elseif opts.Default ~= nil then selected = tostring(opts.Default)
        else selected = values[1] and tostring(values[1]) or "" end
        if type(flag) == "string" and _G.AstroConfig and _G.AstroConfig[flag] ~= nil then
            selected = tostring(_G.AstroConfig[flag])
        end
        local r = makeRow(34)
        makeLabel(r, opts.Title or tostring(flag), 120)
        local b = Instance.new("TextButton")
        b.Size = UDim2.fromOffset(108, 26)
        b.Position = UDim2.new(1, -120, 0.5, -13)
        b.BackgroundColor3 = C.black
        b.Text = (selected ~= "" and selected or "Select") .. " >"
        ...
        b.MouseButton1Click:Connect(function()
            ...
            drop.Size = UDim2.fromOffset(130, math.min(130, math.max(30, #values * 24 + 8)))
            drop.Position = UDim2.new(1, -140, 1, 4)
            drop.BackgroundColor3 = Color3.fromRGB(8, 8, 10)
            drop.BorderSizePixel = 0
            drop.ScrollBarThickness = 2
            drop.CanvasSize = UDim2.new(0, 0, 0, #values * 24 + 8)
            drop.ZIndex = 50
            drop.Parent = r
            corner(drop, 8)
            ...
            for _, v in ipairs(values) do
                ...
                ib.Size = UDim2.new(1, -6, 0, 22)
                ib.BackgroundColor3 = C.row2
                ib.Text = "  " .. tostring(v)
                ...
                ib.MouseButton1Click:Connect(function()
                    selected = tostring(v)
                    b.Text = selected .. " >"
                    if type(flag) == "string" then AstroSave(flag, selected) end
                    if drop then drop:Destroy() drop = nil end
                    task.spawn(function()
                        if multi then pcall(cb, {selected}) else pcall(cb, selected) end
                    end)
                end)
            end
        end)
    ...
    function tabProxy:AddSlider(flag, opts)
        ...
        local minv = opts.Min or 0
        local maxv = opts.Max or 100
        local val = opts.Default or opts.CurrentValue or minv
        if type(flag) == "string" and _G.AstroConfig and _G.AstroConfig[flag] ~= nil then
            val = tonumber(_G.AstroConfig[flag]) or val
        end
        local r = makeRow(34)
        makeLabel(r, opts.Title or tostring(flag), 140)
        local valLbl = Instance.new("TextLabel")
        valLbl.BackgroundTransparency = 1
        valLbl.Size = UDim2.fromOffset(40, 34)
        ...
        bar.Size = UDim2.new(0, 80, 0, 5)
        bar.Position = UDim2.new(1, -136, 0.5, -2)
        bar.BackgroundColor3 = Color3.fromRGB(30, 32, 38)
        bar.BorderSizePixel = 0
        bar.Parent = r
        corner(bar, 3)
        local fill = Instance.new("Frame")
        fill.Size = UDim2.new(math.clamp((val - minv) / math.max(maxv - minv, 1), 0, 1), 0, 1, 0)
        fill.BackgroundColor3 = C.marineHi
        ...
        local function update(x)
            ...
            valLbl.Text = tostring(val)
            if type(flag) == "string" then AstroSave(flag, val) end
    ...
    function tabProxy:AddParagraph(opts)
        ...
        t.Text = opts.Title or "Info"
        t.Parent = r
        local c = Instance.new("TextLabel")
        c.BackgroundTransparency = 1
        c.Size = UDim2.new(1, -16, 0, 14)
        c.Position = UDim2.fromOffset(12, 20)
        c.Font = Enum.Font.Gotham
        c.TextSize = 11
        c.TextXAlignment = Enum.TextXAlignment.Left
        c.TextColor3 = C.muted
        c.Text = opts.Content or opts.Text or ""
        c.Parent = r
        local proxy = {}
        function proxy:SetDesc(text) c.Text = tostring(text or "") end
        function proxy:Set(v)
            if type(v) == "table" then c.Text = tostring(v.Text or v.Content or "")
            else c.Text = tostring(v or "") end
        end
        return proxy
    end
    function tabProxy:AddKeybind() return {} end
    function tabProxy:AddInput(flag, opts)
        ...
        box.Size = UDim2.fromOffset(108, 26)
        ...
        corner(box, 7)
        local cb = opts.Callback or function() end
        box.FocusLost:Connect(function() task.spawn(function() pcall(cb, box.Text) end) end)
        return {}
    end
    return tabProxy
end

function AstroUI:Notify(opts)
    opts = opts or {}
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = opts.Title or "Astro Hub",
...
function Fluent:RegisterCustomTheme() end
function Fluent:SetTheme() end
function Fluent:Notify(opts)
    if _UI and _UI.Notify then _UI:Notify(opts)
    else
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = (opts and opts.Title) or "Astro Hub",
                Text = (opts and (opts.Content or opts.Description)) or "",
...
function Fluent:CreateWindow(opts)
    opts = opts or {}
    -- Prefer real Banana UI library
    if BananaLib and BananaLib.CreateMain then
        local ok, main = pcall(function()
            return BananaLib.CreateMain({
                Title = "Astro Hub",
                Desc = "Blox Fruit",
            })
        end)
        if ok and main and main.CreatePage then
            _BananaMain = main
            Fluent.Window = { Root = nil }
            local windowProxy = {}
            function windowProxy:AddTab(tabOpts)
                tabOpts = tabOpts or {}
                local pageName = tostring(tabOpts.Title or tabOpts.Name or "Tab"):gsub("^Tab ", "")
                local page = main.CreatePage({ Page_Name = pageName, Page_Title = pageName })
                return makeTabProxyFromBanana(page)
            end
            function windowProxy:SelectTab() end
            function windowProxy:Dialog() end
            return windowProxy
        end
    end
    -- Fallback custom Banana-style UI
    _UI = AstroUI.new({ Title = opts.Title or "Astro Hub - Blox Fruit" })
    Fluent.Window = { Root = _UI.main }
    local windowProxy = {}
    function windowProxy:AddTab(tabOpts) return _UI:AddTab(tabOpts) end
    function windowProxy:SelectTab() end
    function windowProxy:Dialog() end
    return windowProxy
end
...
local Tabs = {
    ...
    HopBoss = Window:AddTab({ Title = "Tab Hop Boss", Icon = "lucide/swords" })
}

-- Hop Boss helpers
local function AstroHopServer()
    local cursor = ""
    ...
    for _ = 1, 6 do
        ...
        for _, s in ipairs(data.data) do
            if s.id and tostring(s.id) ~= tostring(JobId) and (s.playing or 0) < (s.maxPlayers or 12) then
                table.insert(servers, tostring(s.id))
            end
        end
    ...
    local function check(folder)
        for _, v in ipairs(folder:GetChildren()) do
            if v.Name == name then
                ...
                if v:FindFirstChild("HumanoidRootPart") then return true end
            end
        end
        return false
    end
    if check(workspace.Enemies) then return true end
    if check(game.ReplicatedStorage) then return true end
    if workspace:FindFirstChild("SeaBeasts") and check(workspace.SeaBeasts) then return true end
    -- elite hunter quest progress
    if name == "Deandre" or name == "Urban" or name == "Diablo" or name == "Elite Hunter" then
        local ok, data = pcall(function()
            return game.ReplicatedStorage.Remotes.CommF_:InvokeServer("EliteHunter", "Progress")
        end)
...
local function StartBossHop(bossName, aliases)
    aliases = aliases or {bossName}
    _G.HopBossTarget = bossName
    _G.HopBossActive = true
    task.spawn(function()
        while _G.HopBossActive and _G.HopBossTarget == bossName do
            local found = false
            for _, n in ipairs(aliases) do
                if BossAlive(n) then found = true break end
            end
            if found then
                pcall(function()
                    Fluent:Notify({Title = "Hop Boss", Content = bossName .. " found on this server!", Duration = 5})
                end)
                _G.HopBossActive = false
                break
            end
            pcall(function()
                Fluent:Notify({Title = "Hop Boss", Content = "Hopping for " .. bossName .. "...", Duration = 2})
            end)
            AstroHopServer()
            task.wait(8)
        end
...
Tabs.HopBoss:AddSection("Hop until boss is on server")
local hopBossList = {
    {"Darkbeard", {"Darkbeard"}},
    {"Dough King", {"Dough King", "Cake Prince"}},
    {"Cake Prince", {"Cake Prince", "Dough King"}},
    {"rip_indra True Form", {"rip_indra True Form", "rip_indra"}},
    {"Elite Hunter", {"Deandre", "Urban", "Diablo"}},
    {"Cursed Captain", {"Cursed Captain"}},
    {"Soul Reaper", {"Soul Reaper"}},
    {"Tyrant of the Skies", {"Tyrant of the Skies"}},
}
for _, entry in ipairs(hopBossList) do
    local title, names = entry[1], entry[2]
    Tabs.HopBoss:AddButton({
        Title = "Hop " .. title,
        Callback = function()
            StartBossHop(title, names)
        end
    })
end
Tabs.HopBoss:AddSection("Moon / Server Hops")
Tabs.HopBoss:AddButton({
    Title = "Hop Fullmoon",
    Callback = function()
        _G.HopFullmoon = true
        _G.HopNearMoon = false
        _G.Hop4HourServer = false
        task.spawn(function()
            while _G.HopFullmoon do
                local okMoon = false
                pcall(function()
                    local lighting = game:GetService("Lighting")
                    -- full moon check (Blox Fruits style)
                    if lighting:GetAttribute("MoonPhase") == 5 or lighting:GetAttribute("MoonPhase") == "Full" then
                        okMoon = true
                    end
                    local moon = lighting:FindFirstChild("Sky") and lighting.Sky
                    if not okMoon and replicated:FindFirstChild("Remotes") then
                        -- some versions expose via world
                        for _, v in ipairs(workspace:GetDescendants()) do
                            if v.Name:lower():find("fullmoon") or v.Name:lower():find("full_moon") then
                                okMoon = true
                                break
                            end
                        end
                ...
                if okMoon then
                    pcall(function() Fluent:Notify({Title="Hop Fullmoon", Content="Fullmoon found!", Duration=4}) end)
                    _G.HopFullmoon = false
                    break
                end
                pcall(function() Fluent:Notify({Title="Hop Fullmoon", Content="Hopping for Fullmoon...", Duration=2}) end)
                AstroHopServer()
                task.wait(8)
            end
        end)
...
Tabs.HopBoss:AddButton({
    ...
    Callback = function()
        ...
        _G.Hop4HourServer = false
        task.spawn(function()
            while _G.HopNearMoon do
                local near = false
                pcall(function()
                    local lighting = game:GetService("Lighting")
                    local phase = lighting:GetAttribute("MoonPhase")
                    -- near full = phase 4 or 5 typically
                    if phase == 4 or phase == 5 or phase == "Almost Full" or phase == "Full" then
                        near = true
                    end
                end)
                if near then
                    pcall(function() Fluent:Notify({Title="Hop Near Moon", Content="Near/Full moon found!", Duration=4}) end)
                    _G.HopNearMoon = false
                    break
                end
                pcall(function() Fluent:Notify({Title="Hop Near Moon", Content="Hopping for Near Moon...", Duration=2}) end)
                AstroHopServer()
                task.wait(8)
            end
        end)
...
Tabs.HopBoss:AddButton({
    Title = "Hop 4 Hours Server",
    Callback = function()
        _G.Hop4HourServer = true
        _G.HopFullmoon = false
        _G.HopNearMoon = false
        task.spawn(function()
            while _G.Hop4HourServer do
                local oldEnough = false
                pcall(function()
                    -- Server age via Workspace DistributedGameTime or job start
                    local age = workspace.DistributedGameTime
                    if age and age >= (4 * 3600) then
                        oldEnough = true
                    end
                    -- alternate: some hubs store server time in attributes
                    if not oldEnough then
                        local st = game.ReplicatedStorage:FindFirstChild("ServerTime") or game.ReplicatedStorage:FindFirstChild("ServerStart")
                        if st and st.Value then
                            local t = typeof(st.Value) == "number" and st.Value or 0
                            if t >= 4 * 3600 then oldEnough = true end
                        end
                    end
                end)
                if oldEnough then
                    pcall(function() Fluent:Notify({Title="Hop 4H Server", Content="Found 4h+ server!", Duration=4}) end)
                    _G.Hop4HourServer = false
                    break
                end
                pcall(function() Fluent:Notify({Title="Hop 4H Server", Content="Hopping for 4h+ server...", Duration=2}) end)
                AstroHopServer()
                task.wait(8)
            end
        end)
...
Tabs.HopBoss:AddButton({
    Title = "Stop All Moon/Server Hops",
    Callback = function()
        _G.HopFullmoon = false
        _G.HopNearMoon = false
        _G.Hop4HourServer = false
        pcall(function() Fluent:Notify({Title="Hop", Content="Moon/Server hops stopped.", Duration=2}) end)
    end
})
Tabs.HopBoss:AddButton({
    Title = "Stop Hop Boss",
    Callback = function()
        ...
        _G.Hop4HourServer = false
        pcall(function()
            Fluent:Notify({Title = "Hop Boss", Content = "All hops stopped.", Duration = 2})
        end)
    end
})
...
Callback = function(Value)
  _B = Value
  pcall(function() AstroSave("Toggle_Bring_Mobs", Value) end)
end})
Tabs.Settings:AddSlider("Slider_Bring_Count", {
Title = "Bring Mob Count",
Description = "How many mobs to bring (1-6)",
Default = 6,
Min = 1,
Max = 6,
Rounding = 1,
Callback = function(Value)
  _G.BringCount = math.clamp(tonumber(Value) or 6, 1, 6)
  pcall(function() AstroSave("Slider_Bring_Count", _G.BringCount) end)
end})
_G.BringCount = _G.BringCount or 6
Tabs.Settings:AddToggle("Toggle_Reset_Teleport", {
Title = "Reset Teleport (Bypass TP)",
Description = "Only when ON the script may use death/reset teleport",
Default = false,
Callback = function(Value)
  _G.ResetTeleport = Value
  pcall(function() AstroSave("Toggle_Reset_Teleport", Value) end)
end})
_G.ResetTeleport = false
Tabs.Settings:AddDropdown("Dropdown_Quest_Mode", {
Title = "Quest Mode",
Description = "",
Values = {"Single Quest", "Double Quest", "Triple Quest"},
Default = "Single Quest",
Callback = function(Value)
  _G.QuestMode = Value
  pcall(function() AstroSave("Dropdown_Quest_Mode", Value) end)
end})
_G.QuestMode = _G.QuestMode or "Single Quest"
...
spawn(function()while wait(Sec)do pcall(function()for a,b in pairs(workspace.Boats:GetChildren())do for c,d in pairs(workspace.Boats[b.Name]:GetDescendants())do if d:IsA("BasePart")then if _G.SailBoats or _G.AutoFindLevi or _G.Prehis_Find or _G.FindMirage or _G.SailBoat_Hydra or _G.AutofindKitIs then d.CanCollide=false else d.CanCollide=true end end end end end)end end)
...
Tabs.SeaEvent:AddToggle("Toggle_Auto_Shark_Anchor", {
...
Tabs.SeaEvent:AddToggle("Toggle_Auto_TTK", {
Title = "Auto TTK",
Description = "",
Default = false,
Callback = function(Value)
...
Tabs.SeaEvent:AddToggle("Toggle_Auto_Find_Levi", {
    Title = "Auto Find Levi",
    Description = "Sails boat only (no combat) until Frozen Dimension spawns, then stops there.",
    Default = false,
    Callback = function(Value)
        _G.AutoFindLevi = Value
        if not Value then
            pcall(function()
                if plr.Character and plr.Character:FindFirstChild("Humanoid") then
...
local FindLeviZones = {
    ...
    CFrame.new(-26779.5215, 30.0005474, -822.858032),
    CFrame.new(-31171.957, 30.0001011, -2256.93774),
    CFrame.new(-34054.6875, 30.2187767, -2560.12012),
    CFrame.new(-38887.5547, 30.0004578, -2162.99023),
    CFrame.new(-44541.7617, 30.0003204, -1244.8584),
}

spawn(function()
    local zoneIndex = 1
    while task.wait() do
        if not _G.AutoFindLevi then
            task.wait(0.3)
            continue
        end
        pcall(function()
            ...
            if frozen then
                local target = frozen.CFrame or CFrame.new(frozen.Position)
                if hum.Sit then
                    _tp(target * CFrame.new(0, 40, 0))
                else
            ...
            if not myBoat then
                local buyBoatCFrame = CFrame.new(-16927.451, 9.086, 433.864)
                TeleportToTarget(buyBoatCFrame)
                if (buyBoatCFrame.Position - hrp.Position).Magnitude <= 12 then
            ...
            local zone = FindLeviZones[zoneIndex]
            if not zone then
                zoneIndex = 1
                zone = FindLeviZones[1]
            end

            _tp(zone * CFrame.new(0, 35, 0))

            if (hrp.Position - zone.Position).Magnitude <= 250 then
                zoneIndex = zoneIndex + 1
                if zoneIndex > #FindLeviZones then
                    zoneIndex = 1
                end
            end
        end)
...
Tabs.SeaEvent:AddSection("Multi Levi")

local MultiLeviPlayerList = {}
for _, v in pairs(game:GetService("Players"):GetPlayers()) do
    if v ~= plr then
        table.insert(MultiLeviPlayerList, v.Name)
    end
end

Tabs.SeaEvent:AddDropdown("Dropdown_Multi_Levi_Players", {
    Title = "Select Multi Levi Players",
    Description = "Host: pick alts that should sit. Alts: pick the host/boat owner.",
    Values = MultiLeviPlayerList,
    Multi = true,
    Default = {},
    Callback = function(Value)
        if type(Value) == "table" then
            _G.MultiLeviPlayers = Value
        elseif Value and Value ~= "" then
            _G.MultiLeviPlayers = {tostring(Value)}
        else
            _G.MultiLeviPlayers = {}
        end
    end
...
Tabs.SeaEvent:AddButton({
    ...
    Callback = function()
        ...
        Fluent:Notify({
            Title = "Multi Levi",
            Content = "Player list refreshed (" .. tostring(#MultiLeviPlayerList) .. ")",
            Duration = 3
        })
    end
})

Tabs.SeaEvent:AddToggle("Toggle_Multi_Levi_Host", {
    Title = "Multi Levi (Host)",
    Description = "Turn on on your main while farming Levi. Keeps your boat ready for alts.",
    Default = false,
    Callback = function(Value)
        _G.MultiLeviHost = Value
    end
})

Tabs.SeaEvent:AddToggle("Toggle_Sit_In_Boat_Multi_Levi", {
    Title = "Sit In Boat Multi Levi",
    Description = "Turn on on alts. They sit in free seats on the host boat.",
    Default = false,
    Callback = function(Value)
        _G.SitMultiLevi = Value
        if not Value then
            pcall(function()
                if plr.Character and plr.Character:FindFirstChild("Humanoid") then
                    plr.Character.Humanoid.Sit = false
...
local function GetBoatByOwnerName(ownerName)
    for _, boat in pairs(workspace.Boats:GetChildren()) do
        local owner = boat:FindFirstChild("Owner")
        if owner and tostring(owner.Value) == tostring(ownerName) then
            return boat
        end
    end
    return nil
...
local function GetFreeBoatSeat(boat)
    ...
    local vs = boat:FindFirstChild("VehicleSeat")
...
local function ResolveHostBoat()
    local targets = _G.MultiLeviPlayers
    if type(targets) == "table" and #targets > 0 then
        for _, name in ipairs(targets) do
            local boat = GetBoatByOwnerName(name)
            if boat then return boat end
        end
    elseif type(targets) == "string" and targets ~= "" then
        local boat = GetBoatByOwnerName(targets)
        if boat then return boat end
    end
    for _, boat in pairs(workspace.Boats:GetChildren()) do
        local owner = boat:FindFirstChild("Owner")
        if owner and tostring(owner.Value) ~= tostring(plr.Name) then
            if GetFreeBoatSeat(boat) then
                return boat
            end
        end
    end
...
spawn(function()
    while task.wait(0.35) do
        if not _G.SitMultiLevi then continue end
        pcall(function()
            local char = plr.Character
            if not char then return end
            local hum = char:FindFirstChildOfClass("Humanoid")
            ...
            local boat = ResolveHostBoat()
            if not boat then return end

            local seat = GetFreeBoatSeat(boat)
            if not seat then return end

            _tp(seat.CFrame * CFrame.new(0, 2, 0))
            task.wait(0.25)
            pcall(function()
                seat:Sit(hum)
            end)
...
spawn(function()
    while task.wait(1) do
        if not _G.MultiLeviHost then continue end
        pcall(function()
            local myBoat = CheckBoat()
            if not myBoat then
                local buyBoatCFrame = CFrame.new(-16927.451, 9.086, 433.864)
                local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then return end
                if (buyBoatCFrame.Position - hrp.Position).Magnitude > 15 then
                    TeleportToTarget(buyBoatCFrame)
                else
                    if _G.SelectedBoat then
                        replicated.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat)
...
function FastAttackModule.GetNearbyTargets(character, folder)
    ...
    table.sort(nearbyTargets, function(a, b)
        ...
        return (ra.Position - characterPosition).Magnitude < (rb.Position - characterPosition).Magnitude
    end)
...
function FastAttackModule.ExecuteFastAttack()
    ...
    -- cap to bring count (default 6), ensure at least multi-hit
    local maxHit = math.clamp(tonumber(_G.BringCount) or 6, 3, 6)
    if #targets > maxHit then
        local trimmed = {}
        for i = 1, maxHit do trimmed[i] = targets[i] end
        targets = trimmed
    end
    ...
    -- fire hit on each primary part so stacked brings all take damage
    for i = 1, math.min(3, #targetParts) do
        local targetHead = targetParts[i][2]
        pcall(function()
...
-- Auto Shark Anchor + Auto TTK loops
task.spawn(function()
  while task.wait(0.35) do
    pcall(function()
      if _G.AutoSharkAnchor then
        local char = plr.Character
        if char then
          local tool = char:FindFirstChild("Shark Anchor") or plr.Backpack:FindFirstChild("Shark Anchor")
          if tool and tool.Parent == plr.Backpack then
            char.Humanoid:EquipTool(tool)
          end
          tool = char:FindFirstChild("Shark Anchor")
          if tool then
            pcall(function()
              tool:Activate()
            end)
            for _, sk in ipairs({"Z", "X", "C"}) do
              pcall(function()
                vim1:SendKeyEvent(true, sk, false, game)
                task.wait(0.03)
                vim1:SendKeyEvent(false, sk, false, game)
              end)
            end
          end
        end
      ...
      if _G.AutoTTK then
        -- TTK: focus Terrorshark / sea threats with skills
        local target = nil
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
          if (v.Name == "Terrorshark" or v.Name == "Shark" or v.Name == "Piranha") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            target = v
            break
          end
        end
        if target and target:FindFirstChild("HumanoidRootPart") then
          _tp(target.HumanoidRootPart.CFrame * CFrame.new(0, 30, 8))
          EquipWeapon(_G.SelectWeapon)
          for _, sk in ipairs({"Z", "X", "C"}) do
            pcall(function()
              vim1:SendKeyEvent(true, sk, false, game)
